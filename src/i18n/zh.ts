...
export const messages: Record<string, string> = {
  ...
  'ai.dir_links.use': '- 本 bot 的卡片页脚显示你正在改代码的仓库和分支。进入某个仓库目录（如新建的 worktree）开始改代码时，先在该目录里执行 `botmux dir use`，否则页脚显示的是会话工作目录的分支。',
  'ai.dir_links.intro': '页脚还会显示当前分支的 {targets} 链接：{steps}命令同样在该仓库目录里执行；只写已确认的真实链接，不要猜；链接变了就重新 set；不要手改 .botmux-dir.json。',
  'ai.dir_links.mr': '创建或找到本任务的 MR 后执行 `botmux dir set mr MR地址`；',
  'ai.dir_links.meego': '确定本任务关联的 Meego 工作项后执行 `botmux dir set meego 工作项地址`；',
